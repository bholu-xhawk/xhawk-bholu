...
router.patch('/todos/:id/done', async (req, res) => {
  const id = Number(req.params.id);
  if (!Number.isInteger(id) || id <= 0) return res.status(400).json({ error: 'Invalid id' });
  const parse = doneSchema.safeParse(req.body || {});
  if (!parse.success) return res.status(422).json({ error: 'Invalid input', details: parse.error.flatten() });
  const done = parse.data.done !== undefined ? parse.data.done : true;
  try {
    const owned = await prisma.todo.findFirst({ where: { id, userId: req.user.id } });
    if (!owned) return res.status(404).json({ error: 'Todo not found' });
    const updated = await prisma.todo.update({ where: { id }, data: { done }, select: selectFields });
    res.json(updated);
  } catch (err) {
    if (err.code === 'P2025') return res.status(404).json({ error: 'Todo not found' });
    res.status(500).json({ error: 'Failed to update todo' });
